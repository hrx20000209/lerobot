...
exec "${PYTHON_BIN}" -u scripts/inference/cosmos_profiled_client.py \
  ...
  --fb_gripper_lead_deg="${FB_GRIPPER_LEAD_DEG:-0}" \
  --fb_fixed_point_secs="${FB_FIXED_POINT_SECS:-0}" \
  --fb_fixed_point_move_deg="${FB_FIXED_POINT_MOVE_DEG:-3.0}" \
  --fb_fixed_point_release_deg="${FB_FIXED_POINT_RELEASE_DEG:-30.0}" \
  --apex="${APEX:-false}" \
  --apex_k1="${APEX_K1:-12.0}" \
  --apex_k2="${APEX_K2:-12.0}" \
  --apex_alpha="${APEX_ALPHA:-3.0}" \
  --apex_gamma_w="${APEX_GAMMA_W:-2e-4}" \
  --apex_gamma_wd="${APEX_GAMMA_WD:-2e-4}" \
  --apex_gamma_j="${APEX_GAMMA_J:-2e-5}" \
  --apex_warmup_secs="${APEX_WARMUP_SECS:-1.5}" \
  --apex_max_correction_deg="${APEX_MAX_CORRECTION_DEG:-12.0}" \
  --reanchor="${REANCHOR:-false}" \
  --reanchor_blend_steps="${REANCHOR_BLEND_STEPS:-8}" \
