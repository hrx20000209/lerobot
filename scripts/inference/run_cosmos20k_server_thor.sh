...
exec "${PYTHON_BIN}" -u -m cosmos_policy.experiments.robot.so101_async_deploy_three_cubes_k16 \
  ...
  --splice_drop_slot="${SPLICE_DROP_SLOT:--1}" \
  --splice_fill="${SPLICE_FILL:-zero}" \
  --generate_future_state="${GENERATE_FUTURE_STATE:-false}" \
  --latent_feedback="${LATENT_FEEDBACK:-false}" \
  --latent_feedback_max_consecutive="${LATENT_FEEDBACK_MAX_CONSECUTIVE:-1}" \
  --latent_feedback_until_s="${LATENT_FEEDBACK_UNTIL_S:-0}" \
  --latent_feedback_measure_all="${LATENT_FEEDBACK_MEASURE_ALL:-false}" \
  --adaptive_sensing="${ADAPTIVE_SENSING:-false}" \
  --adaptive_k_max="${ADAPTIVE_K_MAX:-4}" \
  --adaptive_k_min="${ADAPTIVE_K_MIN:-0}" \
  --adaptive_calibrate_n="${ADAPTIVE_CALIBRATE_N:-6}" \
  --adaptive_value_flat_secs="${ADAPTIVE_VALUE_FLAT_SECS:-15.0}" \
